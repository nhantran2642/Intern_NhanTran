package com.nhantran.utils;

public class Constants {
    public static final String RAILWAY_URL = "http://saferailway.somee.com/";
    public static final String TEMPORARY_MAIL_URL = "https://www.guerrillamail.com";
    public static final String VALID_USERNAME = "nhantran@grr.la";
    public static final String VALID_PASSWORD = "123456789";
    public static final String INVALID_PASSWORD = "0123456789";
    public static final String NON_ACTIVE_USERNAME = "nhantrannotactive@grr.la";
    public static final String NON_ACTIVE_PASSWORD = "1234567890";
}
